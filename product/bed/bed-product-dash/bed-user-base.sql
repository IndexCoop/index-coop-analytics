-- https://dune.xyz/queries/133749

WITH bed_user_base AS (

    WITH bed_address_exposure AS (
    ...
    cohorts AS (
        ...
        SELECT
            ...
            CASE 
                WHEN MAX(exposure) >= 750 THEN '750+'
                WHEN MAX(exposure) >= 250 THEN '250-749'
                WHEN MAX(exposure) >= 50 THEN '50-249'
                WHEN MAX(exposure) >= 10 THEN '10-49'
                ELSE '<10'
            END AS exposure_group
        FROM bed_address_exposure
    ...
    temp AS (
        ...
        SELECT
            ...
            c.cohort,
            c.exposure_group
        FROM bed_address_exposure a
        LEFT JOIN cohorts c ON a.address = c.address
        WHERE a.exposure IS NOT NULL
