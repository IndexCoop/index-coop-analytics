/* dune query here: https://duneanalytics.com/queries/67923
This query compares past 30 days MVI NAV and it's market price by hour.
How it works?
...
(
    ...
    from (
          ...
          from setprotocol_v2."BasicIssuanceModule_evt_SetTokenIssued"
          where "_setToken" IN ('\x72e364f2abdc788b7e918bc238b21f109cd634d7')
          and date_trunc('hour', "evt_block_time") >= (select start_date from start_date_of_past_30_days)
         ) x 
    where x.rnb = 1
)
,
...
select 
	...
	now() - interval '33 days' as start_time_1
