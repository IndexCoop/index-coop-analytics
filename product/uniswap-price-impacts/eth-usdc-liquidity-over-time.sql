-- https://duneanalytics.com/queries/70685

WITH pool as (
select 
    '\x8ad599c3a0ff1de082011efddc58f1908eb6e6d8'::bytea as contract_address,
    6 as dec0,
    18 as dec01
)
...
FROM (
    SELECT
        ...
        SUM(CASE WHEN tick BETWEEN log(1e12/cp.price)/log(1.0001) AND log(1e12/(cp.price/1.02))/log(1.0001) THEN liquidity END)/1e18 AS "Price-2%",
        SUM(CASE WHEN tick BETWEEN log(1e12/(cp.price*1.02))/log(1.0001) AND log(1e12/cp.price)/log(1.0001) THEN liquidity END)/1e18 AS "Price+2%"
