-- https://duneanalytics.com/queries/64545/128645
...
WITH v3_supply AS (
    ...
    where       pool = '\x99ac8cA7087fA4A2A1FB6357269965A2014ABc35'
    )
...
v2_supply AS (
    ...
    WITH v2_pool AS (
      ...
      WHERE pair = '\x004375dff511095cc5a197a54140a24efef3a416'
    ...
    v2_supply AS (
        ...
        INNER JOIN (SELECT * FROM prices.usd WHERE contract_address = '\x2260fac5e5542a773aa44fbcfedf7c193bc2c599') p0 ON date_trunc('minute', s.evt_block_time) = p0.minute
        INNER JOIN (SELECT * FROM prices.usd WHERE contract_address = '\xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48') p1 ON date_trunc('minute', s.evt_block_time) = p1.minute
