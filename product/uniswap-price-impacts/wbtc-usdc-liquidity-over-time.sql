-- https://duneanalytics.com/queries/75898

WITH pool as (
select 
    '\x99ac8ca7087fa4a2a1fb6357269965a2014abc35'::bytea as contract_address
)
...
, mb AS (     --GET ALL MINT AND BURN SUCCESSEFUL CALLS (could also use evt table)
                ...
                AND contract_address =  '\x99ac8ca7087fa4a2a1fb6357269965a2014abc35' --pool address
                ...
                AND contract_address = '\x99ac8ca7087fa4a2a1fb6357269965a2014abc35'
                -- and date_trunc('day', call_block_time) <= '2021-05-06'
            )
...
FROM (
    SELECT
        ...
        SUM(CASE WHEN tick BETWEEN log(1e-2*cp.price/1.02)/log(1.0001) AND log(1e-2*cp.price)/log(1.0001) THEN liquidity END)/1e13 AS "Price-2%",
        SUM(CASE WHEN tick BETWEEN log(1e-2*cp.price)/log(1.0001) AND log(1e-2*cp.price*1.02)/log(1.0001) THEN liquidity END)/1e13 AS "Price+2%"
    FROM liq
    JOIN tp_price as cp ON cp.tp = liq.tp
    GROUP BY 1,2
    ) A
