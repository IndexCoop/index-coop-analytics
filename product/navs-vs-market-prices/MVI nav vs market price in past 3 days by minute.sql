/* dune query here: https://duneanalytics.com/queries/65732
...
2.figures out the composition of per MVI each minute,then times the price of each composition at that time ,adds them up gets the mvi nav
3.compares mvi nav with mvi market price 
*/

 
with start_date_of_past_3_days as
(
    select
...
(
    ...
    from (
          ...
          from setprotocol_v2."BasicIssuanceModule_evt_SetTokenIssued"
          where "_setToken" IN ('\x72e364f2abdc788b7e918bc238b21f109cd634d7')
          and date_trunc('minute', "evt_block_time") >= (select start_date from start_date_of_past_3_days)
         ) x 
    where x.rnb = 1
)
,
...
(
    ...
    from (
          select
             ...
             (select max(minute_date) from index_composition ) as last_mint_time
...
(
    ...
    from generate_series( (select min(minute_date) from index_composition),
                          now(),
                          interval  '1 min'
                         ) t(x)
),

/*index composition by minute*/
index_composition_by_minute as
(
    select
        contract_address,
        price_time,
...
(
    select 
        now() - interval '3 days' as start_time,
    	now() - interval '6 days' as start_time_1
...
( 
    SELECT
        ...
        sw."evt_tx_hash",
        date_trunc('minute', sw."evt_block_time") AS hour
...
(
    ...
    from index_composition_by_minute a
    left join price_by_minute b
...
from index_nav a
left join (select * from price_by_minute where symbol='MVI') b
