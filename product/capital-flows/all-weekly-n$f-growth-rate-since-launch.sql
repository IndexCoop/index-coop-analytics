...
-- QUERY:       https://dune.xyz/queries/181927/340225
...
SELECT
*,
sum(net_inflow) OVER (PARTITION BY name ORDER BY day ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_inflows
FROM aum
),

_cum_net_inflows_days AS (
SELECT
    c.day AS day,
    c.name,
    c.aum,
    c.cumulative_inflows,
    EXTRACT('days' FROM c.day - p.inception_date) AS days_since_launch,
    SUM(c.net_inflow) OVER (PARTITION BY c.name ORDER BY day ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) / LAG(c.cumulative_inflows, 7) OVER (PARTITION BY c.name ORDER BY day) AS seven_day_flows
FROM cum_net_inflows c
LEFT JOIN index_products p ON c.name = p.name
),

cum_net_inflows_days AS (
SELECT
    day,
    days_since_launch::INTEGER,
    days_since_launch::INTEGER / 7 AS week,
    name,
    AVG(seven_day_flows) OVER (PARTITION BY name ORDER BY day ROWS BETWEEN 14 PRECEDING AND 14 FOLLOWING) AS seven_day_flows
FROM _cum_net_inflows_days
)

SELECT * FROM cum_net_inflows_days
WHERE days_since_launch >= 56 AND MOD(days_since_launch,7) = 0
ORDER BY days_since_launch ASC
