-- https://duneanalytics.com/queries/34735
...
SELECT
    ...
    COALESCE(d.amount, 0) AS index_rewarded,
    COALESCE(d.amount * x.price, 0) AS cost_of_index_rewarded,
    ...
    p.price AS dpi_price,
    x.price AS index_price
...
LEFT JOIN dpi_daily_index_rewards d ON t.day = d.evt_block_day
LEFT JOIN dpi_daily_price_feed AS p ON t.day = p.dt
LEFT JOIN index_price_feed AS x ON t.day = x.dt
