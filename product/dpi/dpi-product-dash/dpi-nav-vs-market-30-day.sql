/* dune query here: https://duneanalytics.com/queries/67903
This query compares past 30 days DPI NAV and it's market price hourly.
How it works?
...
/*DPI minted amount each time,only keeps one every hour in  past 30 days */
with start_date_of_past_30_days as
(
    select
        max(hour_date) as start_date
    from (
           select
                "evt_tx_hash",
                "evt_block_time",
...
(
    ...
    where x.rnb = 1
      and hour_date >= (select start_date from start_date_of_past_30_days)
)
,
...
select
    ...
    a.DPI_market_price,
    b.DPI_NAV,
    ((a.dpi_market_price - b.DPI_NAV) / b.DPI_NAV) AS premium_discount_percentage
from DPI_market_price_t a 
left join DPI_NAV_t b
on a.hour_date = b.price_time
where a.hour_date >= now() - interval '30 days'
