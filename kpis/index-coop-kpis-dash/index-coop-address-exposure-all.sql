...
, wallet AS (
    ...
    WITH wallet AS (
        ...
        SELECT
            ...
            amount_raw / 10^18 AS amount
        FROM erc20."token_balances"
        WHERE token_address = '\x0b498ff89709d3838a063f1dfa463091f9801c2b'
        ORDER BY date_trunc('day', "timestamp"), wallet_address, "timestamp" DESC
...
, wallet AS (
    ...
    WITH wallet AS (
        ...
        SELECT
            ...
            amount_raw / 10^18 AS amount
        FROM erc20."token_balances"
        WHERE token_address = '\x72e364f2abdc788b7e918bc238b21f109cd634d7'
        ORDER BY date_trunc('day', "timestamp"), wallet_address, "timestamp" DESC
...
, wallet AS (
    ...
    WITH wallet AS (
        ...
        SELECT
            ...
            amount_raw / 10^18 AS amount
        FROM erc20."token_balances"
        WHERE token_address = '\x2af1df3ab0ab157e1e2ad8f88a7d04fbea0c7dc6'
        ORDER BY date_trunc('day', "timestamp"), wallet_address, "timestamp" DESC
    ...
    days AS (
        ...
        SELECT generate_series('2021-07-21'::timestamp, date_trunc('day', NOW()), '1 day') AS dt -- Generate all days
