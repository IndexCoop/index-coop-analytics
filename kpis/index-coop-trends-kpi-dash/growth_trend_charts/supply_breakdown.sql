...
supply_breakdown_7d_ma as (

SELECT 
    *,
    AVG(total) OVER (ORDER BY day ROWS BETWEEN 7 PRECEDING AND CURRENT ROW) AS avg_total_supply,
    AVG(incentivized) OVER (ORDER BY day ROWS BETWEEN 7 PRECEDING AND CURRENT ROW) AS avg_incentivized_supply,
    AVG(unincentivized) OVER (ORDER BY day ROWS BETWEEN 7 PRECEDING AND CURRENT ROW) AS avg_unincentivized_supply,
    AVG(liquidity) OVER  (ORDER BY day ROWS BETWEEN 7 PRECEDING AND CURRENT ROW) as avg_liquidity
FROM daily_supply_breakdown

),

lagged_supply_breakdown_7d_ma as (

SELECT day,
       avg_total_supply,
       avg_incentivized_supply,
       avg_unincentivized_supply,
       avg_liquidity,
       lag(avg_total_supply, 7) over (order by day) as avg_total_supply_last_week,
       lag(avg_total_supply, 30) over (order by day) as avg_total_supply_last_month,
       lag(avg_incentivized_supply, 7) over (order by day) as avg_incentivized_supply_last_week,
       lag(avg_incentivized_supply, 30) over (order by day) as avg_incentivized_supply_last_month,
       lag(avg_unincentivized_supply, 7) over (order by day) as avg_unincentivized_supply_last_week,
       lag(avg_unincentivized_supply, 30) over (order by day) as avg_unincentivized_supply_last_month,
       lag(avg_liquidity, 7) over (order by day) as avg_liquidity_last_week,
       lag(avg_liquidity, 30) over (order by day) as avg_liquidity_last_month
FROM supply_breakdown_7d_ma
...
SELECT day,
       ((avg_total_supply - avg_total_supply_last_week) / avg_total_supply_last_week)::numeric as total_supply_week_over_week_change, 
       ((avg_total_supply - avg_total_supply_last_month) / avg_total_supply_last_month)::numeric as total_supply_month_over_month_change,
       ((avg_incentivized_supply - avg_incentivized_supply_last_week) / avg_incentivized_supply_last_week)::numeric as incentivized_supply_week_over_week_change, 
       ((avg_incentivized_supply - avg_incentivized_supply_last_month) / avg_incentivized_supply_last_month)::numeric as incentivized_supply_month_over_month_change,
       ((avg_unincentivized_supply - avg_unincentivized_supply_last_week) / avg_unincentivized_supply_last_week)::numeric as unincentivized_supply_week_over_week_change, 
       ((avg_unincentivized_supply - avg_unincentivized_supply_last_month) / avg_unincentivized_supply_last_month)::numeric as unincentivized_supply_month_over_month_change,
       ((avg_liquidity - avg_liquidity_last_week) / avg_liquidity_last_week)::numeric as liquidity_week_over_week_change, 
       ((avg_liquidity - avg_liquidity_last_month) / avg_liquidity_last_month)::numeric as liquidity_month_over_month_change
FROM lagged_supply_breakdown_7d_ma
