...
lagged_daily_revenue as (

SELECT day,
       revenue,
       lag(revenue, 7) over (order by day) as revenue_last_week,
       lag(revenue, 30) over (order by day) as revenue_last_month
FROM daily_revenue
...
SELECT day,
       revenue,
       revenue_last_week,
       ((revenue - revenue_last_week) / revenue_last_week)::numeric as week_over_week_change, 
       ((revenue - revenue_last_month) / revenue_last_month)::numeric as month_over_month_change 
FROM lagged_daily_revenue

),

revenue_wow_rolling as (
...
select day, 
       week_over_week_change as "WoW %",
       month_over_month_change as "MoM %"
from revenue_wow_rolling
