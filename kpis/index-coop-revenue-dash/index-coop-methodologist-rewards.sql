-- https://duneanalytics.com/queries/46937/92498
...
data AS (
    ...
    SELECT 
        ...
        'Titans of Data' AS methodologist,
        SUM(amount) OVER (ORDER BY day) AS units
    FROM data_units
...
SELECT DISTINCT * FROM data_revenue
...
SELECT
    to_char(day, 'Mon') || ' ' || date_part('year', day) AS month,
    methodologist,
...
SELECT 
*,
SUM(revenue) OVER (PARTITION BY month ORDER BY month) AS monthly_revenue,
revenue / SUM(revenue) OVER (PARTITION BY month ORDER BY month) AS revenue_cut,
41666 * (revenue / SUM(revenue) OVER (PARTITION BY month ORDER BY month)) AS index_payout
FROM t
...
SELECT
    month AS "Month",
    methodologist AS "Methodologist",
    ROUND(revenue::numeric,2) AS "Revenue Contributed",
    ROUND(revenue_cut::numeric,3) AS "% Contribution",
    ROUND(index_payout::numeric,2) AS "Methodologist Reward ($INDEX)"
FROM calc